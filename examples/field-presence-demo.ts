...
} from '../src/logic/fieldNormalization';
...
for (const [normalizedField, presence] of presenceMap) {
  ...
  console.log(
    `  Present on both: ${isFieldPresentOnBothSides(normalizedField, presenceMap) ? '✅' : '❌'}`
  );
  console.log('');
}
...
console.log(
  `Fields only on applicant side: ${fieldsOnlyOnApplicant.join(', ')}`
);
console.log(
  `Fields only on counterparty side: ${fieldsOnlyOnCounterparty.join(', ')}`
);
